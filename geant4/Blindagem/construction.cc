...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    G4Material *m_plastico = new G4Material("Plastico ABS", 1.04*g/cm3, 3);
    m_plastico->AddElement(elH, 0.08);
    m_plastico->AddElement(elC, 0.848);
    m_plastico->AddElement(elN_puro, 0.072);


    // Areia
    G4Material* m_areia = new G4Material("Areia", 2.65*g/cm3, 2);
    m_areia->AddElement(elSi, 1);
    m_areia->AddElement(elO, 2);
